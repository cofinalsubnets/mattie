(library (mattie parsers language)
  ...
  (define (paren s st) ((cat (term "(") ws* prec4 ws* (term ")")) s st))
  (define dot (tag-nullary 'dot (term ".")))
  (define (prec0 s st)
    ((alt paren term- dot (conj (comp defn) word)) s st))
  (define rep- (tag-unary 'rep (cat prec0 ws* (term "*"))))
  (define opt- (tag-unary 'opt (cat prec0 ws* (term "?"))))
  (define neg- (tag-unary 'neg (cat prec0 ws* (term "~"))))
  (define prec1 (alt rep- opt- neg- prec0))
  (define (prec2 s st) ((disj cat- prec1) s st))
  (define (prec3 s st) ((disj and- prec2) s st))
  (define (prec4 s st) ((disj alt- prec3) s st))
  ...
  (define defn (tag-binary 'def (cat ws* word ws* (term "<-") ws* prec4)))
